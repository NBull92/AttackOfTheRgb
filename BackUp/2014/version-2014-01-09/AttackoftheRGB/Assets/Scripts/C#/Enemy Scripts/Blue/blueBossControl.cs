...
public class blueBossControl : MonoBehaviour 
{
	...
	{ 			
		if(health.health <= 0)
		{
			...
			if(myTransform.position.x > (LookAtTarget.position.x + 48) || myTransform.position.x < (LookAtTarget.position.x - 48)
				|| myTransform.position.z > (LookAtTarget.position.z + 48) || myTransform.position.z < (LookAtTarget.position.z - 48))
			{
	...
	{		
		...
		{
			savedTime = seconds;		
			Transform bullet = (Transform)Instantiate(BIGbulletPrefab, transform.Find("Arm1").Find("spawnPoint1").transform.position, Quaternion.identity);
			bullet.gameObject.tag = "blueBossProjectile";		
			bullet.rigidbody.AddForce(transform.forward * 2500);
			
			Transform bullet2 = (Transform)Instantiate(BIGbulletPrefab, transform.Find("Arm2").Find("spawnPoint2").transform.position, Quaternion.identity);
		...
		else
		{
			...
			{
				savedTime = seconds;		
				Transform bullet = (Transform)Instantiate(bulletPrefab, transform.Find("CentreArm").Find("spawnPoint3").transform.position, Quaternion.identity);
				bullet.gameObject.tag = "blueBossProjectile";		
				bullet.rigidbody.AddForce(transform.forward * 2500);
				
				Transform bullet2 = (Transform)Instantiate(bulletPrefab, transform.Find("CentreArmRight").Find("spawnPoint4").transform.position, Quaternion.identity);
				bullet2.gameObject.tag = "blueBossProjectile";		
				bullet2.rigidbody.AddForce(transform.forward * 2500);
				
				Transform bullet3 = (Transform)Instantiate(bulletPrefab, transform.Find("CentreArmLeft").Find("spawnPoint5").transform.position, Quaternion.identity);
				bullet3.gameObject.tag = "blueBossProjectile";		
				bullet3.rigidbody.AddForce(transform.forward * 2500);
			}
		}
	}
}
